...
- 커밋 제목의 타입은 일반적인 영문 Conventional Commits 형식을 사용하고, 변경 요약과 본문은 한글로 작성한다. 라이브러리명, 명령어, 파일명처럼 고유한 기술 용어는 원문 표기를 허용한다.
- 제목은 `<type>: <한글 변경 요약>` 형식으로 작성한다.
- 타입은 `feat`(기능), `fix`(버그 수정), `mod`(일반 수정), `docs`(문서), `test`(테스트), `refactor`(리팩터링), `chore`(설정·기타), `style`(코드 형식), `perf`(성능), `build`(빌드), `ci`(CI) 중 작업 성격에 맞는 값을 선택한다.
