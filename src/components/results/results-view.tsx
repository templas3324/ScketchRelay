...
export function ResultsView({ code }: { code: string }) {
  ...
  useEffect(() => {
    ...
    return () => { window.clearInterval(poller); if (channel) void channel.unsubscribe(); };
  }, [code, refresh]);
  ...
  return <main className="min-h-screen bg-[#fff8e8] px-5 py-8 text-[#272334]"><div className="mx-auto max-w-4xl"><header className="flex items-center justify-between gap-3"><Link href="/" className="text-xl font-black">✏️ Scketch Relay</Link><GameGuideButton /></header><section className="mt-10 text-center"><p className="text-5xl">🎉</p><h1 className="mt-3 text-4xl font-black">우리의 엉뚱한 릴레이</h1><p className="mt-3 font-bold text-[#71697b]">{snapshot.revealedCount} / {snapshot.totalRelays}개 공개</p></section>
