...
export function FirstSentence({ code }: { code: string }) {
  ...
  useEffect(() => {
    // Realtime 재연결 중 변경 신호를 놓쳐도 현재 라운드를 결국 복구하도록 낮은 빈도로 동기화한다.
    const poller = window.setInterval(() => void refresh().catch(() => undefined), 10_000);
  ...
  return <main className="min-h-screen bg-[#fff8e8] px-5 py-8 text-[#272334]"><div className="mx-auto max-w-3xl"><header className="flex items-center justify-between gap-3"><Link href="/" className="text-xl font-black">✏️ Scketch Relay</Link><div className="flex items-center gap-2"><GameGuideButton />{!isFinished && <span className="rounded-full bg-[#ffe17b] px-3 py-1 font-black">{secondsLeft}초</span>}</div></header><section className="mt-10 rounded-[32px] border-[3px] border-[#272334] bg-white p-6 text-center shadow-[9px_10px_0_#272334] sm:p-10">
